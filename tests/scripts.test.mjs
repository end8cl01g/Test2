...
test('CI workflow 必須先驗證再佈署，且部署只在 main', () => {
  ...
  const hasWorkflow = existsSync(p);
  if (hasWorkflow) {
  ...
  if (hasGit) {
    ...
    for (const f of [...files, 'gas/dist/Code.gs', 'build/app.js']) {
      try {
        ...
        // Ignored check exits with 1 if not ignored; or ignore error if git fails
      }
    }
  }
});
...
test('.githooks 內容齊備且有自愈路徑（mode 位會被容器還原洗掉，硬閘門是 CI）', () => {
  const hasGithooks = existsSync(join(ROOT, '.githooks'));
...
test('gas/.clasp.template.json 是入庫的政策來源（CI 上沒有 .clasp.json 也要能驗 rootDir）', () => {
  ...
  if (hasGit) {
    ...
    try {
      ...
      // If check-ignore exits with non-zero when not ignored or other git error, we handle it
    }
  }
...
test('deps.mjs 不只裝套件，也會補齊產物（CI 上 check 跑在 build 之前）', () => {
  ...
  assert.doesNotMatch(src, /need\('build\/app\.js'/, '舊前端（rollup → build/app.js）已捨棄：deps 不該再去補它');
  assert.match(src, /npx rollup -c rollup\.gas\.config\.mjs|npx rollup -c/, '要直接叫 rollup，不要用 npm run build（會遞迴回 deps.mjs）');
});


test('CI 的「外部 CDN」檢查要掃 URL、掃檔要涵蓋 manifest（實測：掃單字會被註解誤判，紅了整個 Actions）', () => {
  const hasWorkflow = existsSync(join(ROOT, '.github', 'workflows', 'deploy.yml'));
  if (hasWorkflow) {
    const y = readFileSync(join(ROOT, '.github', 'workflows', 'deploy.yml'), 'utf8');
    ...
    assert.match(cdn, /manifest\.json/, 'manifest 也要掃（外部字体/資源最常藏在那）');
  }
