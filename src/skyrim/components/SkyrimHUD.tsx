...
export const SkyrimHUD: React.FC<{ stats?: HudStats }> = ({ stats = {} as HudStats }) => {
  const s = stats || {};
  const [magicka, setMagicka] = useState<number>(s.magicka ?? 380);
  const [maxMagicka] = useState<number>(s.maxMagicka ?? 380);

  const [health, setHealth] = useState<number>(s.health ?? 520);
  const [maxHealth] = useState<number>(s.maxHealth ?? 520);

  const [stamina, setStamina] = useState<number>(s.stamina ?? 340);
  const [maxStamina] = useState<number>(s.maxStamina ?? 340);
