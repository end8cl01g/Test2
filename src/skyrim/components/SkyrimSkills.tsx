...
import { unlockSkillNode } from '../store';

export const SkyrimSkills: React.FC<{
  trees?: SkyrimSkillTree[];
  perkPoints?: number;
  ...
  // Keep state synchronized with database props
  useEffect(() => {
    if (initialUnlocked) {
      setUnlockedPerks({ ...initialUnlocked });
    }
  }, [initialUnlocked]);

  useEffect(() => {
    if (initialPoints !== undefined) {
      setPerkPoints(initialPoints);
    }
  }, [initialPoints]);
  ...
  const canUnlockDirectly = (perk: SkyrimPerk): boolean => {
    ...
    const depsMet = (perk.requires || []).every(r => !!unlockedPerks[r]);
    // Check XP requirement
    const xpMet = playerXP >= (perk.minXp ?? 0);
    // Check Streak requirement
    const streakMet = playerStreak >= (perk.minStreak ?? 0);
    
    return depsMet && xpMet && streakMet;
  };

  // Helper: find all missing prerequisites in chain (topological order)
  const getMissingPrereqChain = (perk: SkyrimPerk): SkyrimPerk[] => {
    ...
    const dfs = (pId: string) => {
      ...
      const p = SKYRIM_SKILL_TREES.flatMap(t => t.perks).find(node => node.id === pId);
      if (!p) return;
      
      if (!unlockedPerks[p.id]) {
        (p.requires || []).forEach(rId => dfs(rId));
        if (!missing.some(m => m.id === p.id)) {
          missing.push(p);
        }
      }
    };
    
    (perk.requires || []).forEach(rId => dfs(rId));
    return missing;
  };
  ...
  const handleInvestPerk = (perk: SkyrimPerk) => {
    ...
    // Check prerequisite
    const missingDeps = (perk.requires || []).filter(r => !unlockedPerks[r]);
    if (missingDeps.length > 0) {
      const firstMissingId = missingDeps[0];
      const firstMissingPerk = SKYRIM_SKILL_TREES.flatMap(t => t.perks).find(p => p.id === firstMissingId);
      const missingName = firstMissingPerk ? firstMissingPerk.name : firstMissingId;

      setHighlightedPerkId(firstMissingId);
      setTimeout(() => setHighlightedPerkId(null), 2500);

      setFeedbackMessage({
        text: `無法直接解鎖【${perk.name}】：必須先點亮前置天賦星宿【${missingName}】！`,
        type: 'warning'
      });
      skyrimAudio.playMenuClick();
      return;
    ...
    // Check XP requirement
    if (playerXP < (perk.minXp ?? 0)) {
      setFeedbackMessage({
        text: `解鎖失敗：您的技能經驗 (XP) 不足！需要 ${perk.minXp}，目前僅有 ${playerXP}。`,
        type: 'warning'
      });
      skyrimAudio.playMenuClick();
      return;
    ...
    // Check Streak requirement
    if (playerStreak < (perk.minStreak ?? 0)) {
      setFeedbackMessage({
        text: `解鎖失敗：您的連續鍛鍊天數 (Streak) 不足！需要 ${perk.minStreak} 天，目前僅有 ${playerStreak} 天。`,
        type: 'warning'
      });
      skyrimAudio.playMenuClick();
      return;
    ...
    // Successful unlock!
    if (onPerkUnlocked) {
      unlockSkillNode(perk.id).then((res) => {
        if (res.ok) {
          setUnlockedPerks(prev => ({ ...prev, [perk.id]: true }));
          setPerkPoints(prev => prev - 1);
          setFeedbackMessage({
            text: `★ 璀璨星辰點亮！成功習得天賦：【${perk.name}】（${perk.ranks}）！已加 50 經驗！`,
            type: 'success'
          });
          skyrimAudio.playArcaneChime();
        ...
        setPerkPoints(prev => prev - 1);
        setFeedbackMessage({
          text: `★ 璀璨星辰點亮！(本機) 成功習得天賦：【${perk.name}】（${perk.ranks}）！`,
          type: 'success'
        });
        skyrimAudio.playArcaneChime();
      });
  ...
  const handleChainUnlock = (perk: SkyrimPerk) => {
    ...
    // Check XP and Streak requirements for final perk and all missing prerequisites
    for (const p of [...missingChain, perk]) {
      if (playerXP < (p.minXp ?? 0)) {
        setFeedbackMessage({
          text: `連鎖點亮失敗：【${p.name}】需要技能經驗 (XP) ${p.minXp}，您目前僅有 ${playerXP}。`,
          type: 'warning'
        });
        skyrimAudio.playMenuClick();
        return;
      }
      if (playerStreak < (p.minStreak ?? 0)) {
        setFeedbackMessage({
          text: `連鎖點亮失敗：【${p.name}】需要連續鍛鍊 ${p.minStreak} 天，您目前僅有 ${playerStreak} 天。`,
          type: 'warning'
        });
        skyrimAudio.playMenuClick();
        return;
    ...
    if (onPerkUnlocked) {
      const chainToUnlock = [...missingChain, perk];
      const runChain = async () => {
        for (const p of chainToUnlock) {
          const res = await unlockSkillNode(p.id);
          if (!res.ok) {
            throw new Error(`點亮【${p.name}】時出錯: ${res.why}`);
          }
        }
      };
      ...
      runChain().then(() => {
        ...
        chainToUnlock.forEach(p => {
          updated[p.id] = true;
        });
        setUnlockedPerks(updated);
        setPerkPoints(prev => prev - totalNeeded);
        setFeedbackMessage({
          text: `★ 奧術共鳴！連鎖點亮了 ${missingChain.map(p => p.name).join(' ➔ ')} ➔ 【${perk.name}】！已加 ${chainToUnlock.length * 50} 經驗！`,
          type: 'success'
        });
        skyrimAudio.playArcaneChime();
        onPerkUnlocked(perk.id);
  ...
  const handleRefundPerk = (perk: SkyrimPerk) => {
    ...
    let crossDependent: SkyrimPerk | undefined = undefined;
    for (const tree of SKYRIM_SKILL_TREES) {
      const dep = tree.perks.find(p => (p.requires || []).includes(perk.id) && unlockedPerks[p.id]);
      if (dep) {
        crossDependent = dep;
        break;
      }
    ...
    if (crossDependent) {
      setFeedbackMessage({
        text: `無法收回點數：後續已點亮天賦【${crossDependent.name}】依賴此星宿，請先收回後續天賦！`,
        type: 'warning'
      });
      skyrimAudio.playMenuClick();
      return;
  ...
  const isPrereqMet = (selectedPerk.requires || []).every(r => !!unlockedPerks[r]);
    ...
    <section className="relative w-full py-6 px-3 sm:px-6 md:px-8 max-w-6xl mx-auto flex flex-col items-center">
      ...
      <div className="relative w-full h-[460px] sm:h-[500px] md:h-[520px] rounded-2xl border-2 border-stone-800 bg-gradient-to-b from-[#05060d] via-[#090b16] to-[#030408] shadow-[0_25px_60px_rgba(0,0,0,0.95)] overflow-hidden select-none mb-4">
        ...
        {activeSkillTree.perks.map(perk => {
          ...
          const hasPrereq = (perk.requires || []).every(r => !!unlockedPerks[r]);
          const xpMet = playerXP >= (perk.minXp ?? 0);
          const streakMet = playerStreak >= (perk.minStreak ?? 0);
          const canUnlock = !isUnlocked && hasPrereq && xpMet && streakMet && perkPoints > 0;
        ...
        <div className="flex-1 text-left">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span className={`text-[10px] font-cinzel px-2.5 py-0.5 rounded-full border font-bold ${
              playerXP >= (selectedPerk.minXp ?? selectedPerk.skillReq)
                ? 'bg-emerald-950/70 text-emerald-300 border-emerald-600/50'
                : 'bg-red-950/70 text-red-300 border-red-600/50'
            }`}>
              經驗需求 (XP REQ)：{selectedPerk.minXp ?? selectedPerk.skillReq} (目前 {playerXP})
            </span>

            {/* Streak Req Badge */}
            {(selectedPerk.minStreak ?? 0) > 0 && (
              <span className={`text-[10px] font-cinzel px-2.5 py-0.5 rounded-full border font-bold ${
                playerStreak >= (selectedPerk.minStreak ?? 0)
                  ? 'bg-emerald-950/70 text-emerald-300 border-emerald-600/50'
                  : 'bg-red-950/70 text-red-300 border-red-600/50'
              }`}>
                連續天數需求：{selectedPerk.minStreak} 天 (目前 {playerStreak} 天)
              </span>
            )}
