...
export const ANCIENT_SCROLLS: ScrollItem[] = (W.phases ? Object.entries<any>(W.phases) : []).flatMap(
  ([pid, p]: [string, any], pi: number) => Object.entries<any>(p.days || {}).map(([dayKey, day]: [string, any], di: number) => {
    ...
    const exercises: any[] = Array.isArray(day) ? day : (day.exercises || day.items || []);
    const lines = exercises.map((e: any) => {
      const dose = e.detail || [e.sets && `${e.sets} 組`, e.reps && `× ${e.reps}`, e.seconds && `${e.seconds} 秒`].filter(Boolean).join(' ');
